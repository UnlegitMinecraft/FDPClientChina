    ...
    override fun execute(args: Array<String>) {
        if (args.size > 1) {
            ...
            when (arg1.lowercase()) {
                "add" -> {
                    if (args.size > 3) {
                        ...
                        if (key != Keyboard.KEY_NONE) {
                            ...
                            FDPClientChina.macroManager.macros.add(Macro(key, comm))
                ...
                "remove" -> {
                    if (args.size > 2) {
                        if (args[2].startsWith(".")) {
                            FDPClientChina.macroManager.macros.filter { it.command == StringUtils.toCompleteString(args, 2) }
                        } else {
                            val key = Keyboard.getKeyIndex(args[2].uppercase())
                            FDPClientChina.macroManager.macros.filter { it.key == key }
                        }.forEach {
                            FDPClientChina.macroManager.macros.remove(it)
                ...
                "list" -> {
                    alert("Macros:")
                    FDPClientChina.macroManager.macros.forEach {
