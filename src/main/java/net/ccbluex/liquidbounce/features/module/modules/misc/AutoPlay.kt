...
class AutoPlay : Module() {
            ...
            when (modeValue.get().lowercase()) {
                ...
                "blocksmc" -> {
                    if (clickState == 1 && text.contains("Only VIP players can join full servers!", true)) {
                        FDPClientChina.hud.addNotification(Notification(this.name, "Join failed! trying again...", NotifyType.WARNING, 3000))
                ...
                "minefc/heromc_bedwars" -> {
                    ...
                    if (showGuiWhenFailedValue.get() && text.contains("giây", false) && text.contains("thất bại", false)) {
                        FDPClientChina.hud.addNotification(Notification(this.name, "Failed to join, showing GUI...", NotifyType.ERROR, 1000))
                ...
                "supercraft" -> {
                    ...
                    if (text.contains("El juego ya fue iniciado.", true)) {
                        FDPClientChina.hud.addNotification(Notification(this.name, "Failed to join, retrying...", NotifyType.ERROR, 1755))
                        queueAutoPlay {
                            mc.thePlayer.sendChatMessage("/sw leave")
                            mc.thePlayer.sendChatMessage("/sw randomjoin solo")
                        }
    ...
    private fun queueAutoPlay(delay: Long = delayValue.get().toLong() * 1000, runnable: () -> Unit) {
        ...
        if (this.state) {
            ...
            FDPClientChina.hud.addNotification(Notification(this.name, "Sending you to next game in ${delayValue.get()}s...", NotifyType.INFO, delayValue.get() * 1000))
        }
    }
