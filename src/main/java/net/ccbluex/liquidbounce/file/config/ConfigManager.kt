...
class ConfigManager {
    private val configSetFile = File(FDPClientChina.fileManager.dir, "config-settings.json")

    private val sections = mutableListOf<ConfigSection>()

    var nowConfig = "default"
    private var nowConfigInFile = "default"
    var configFile = File(FDPClientChina.fileManager.configsDir, "$nowConfig.json")
    var needSave = false
    ...
    fun load(name: String, save: Boolean = true) {
        ...
        nowConfig = name
        configFile = File(FDPClientChina.fileManager.configsDir, "$nowConfig.json")
    ...
    fun save(saveConfigSet: Boolean = nowConfigInFile != nowConfig, forceSave: Boolean = false) {
        if (FDPClientChina.isLoadingConfig && !forceSave) {
            return
        }
    ...
    fun loadLegacySupport() {
        if (FDPClientChina.fileManager.loadLegacy()) {
            if (File(FDPClientChina.fileManager.configsDir, "$nowConfig.json").exists()) {
                nowConfig = "legacy"
                configFile = File(FDPClientChina.fileManager.configsDir, "$nowConfig.json")
                save(forceSave = true)
            } else {
            ...
            script.lines().filter { it.isNotEmpty() && !it.startsWith('#') }.forEachIndexed { _, s ->
                ...
                when (args[0]) {
                    ...
                    "targetDead" -> {
                        ...
                        val value = args[2]
                        val module = FDPClientChina.moduleManager.getModule(moduleName) ?: return@forEachIndexed
        ...
        val oldSettingDir = File(FDPClientChina.fileManager.dir, "settings")
        if (oldSettingDir.exists()) {
            oldSettingDir.listFiles().forEach {
                ...
                if (!FDPClientChina.fileManager.legacySettingsDir.exists()) {
                    FDPClientChina.fileManager.legacySettingsDir.mkdir()
                }

                it.renameTo(File(FDPClientChina.fileManager.legacySettingsDir, it.name))
            }
