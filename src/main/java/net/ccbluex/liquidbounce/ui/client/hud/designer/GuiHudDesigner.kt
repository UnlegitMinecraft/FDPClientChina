...
class GuiHudDesigner : GuiScreen() {
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        FDPClientChina.hud.render(true, partialTicks)
        FDPClientChina.hud.handleMouseMove(mouseX, mouseY)

        if (!FDPClientChina.hud.elements.contains(selectedElement)) {
        ...
        if (wheel != 0) {
            for (element in FDPClientChina.hud.elements) {
    ...
    override fun mouseClicked(mouseX: Int, mouseY: Int, mouseButton: Int) {
        ...
        FDPClientChina.hud.handleMouseClick(mouseX, mouseY, mouseButton)
    ...
    override fun mouseReleased(mouseX: Int, mouseY: Int, state: Int) {
        ...
        FDPClientChina.hud.handleMouseReleased()
    }

    override fun onGuiClosed() {
        Keyboard.enableRepeatEvents(false)
        FDPClientChina.fileManager.saveConfig(FDPClientChina.fileManager.hudConfig)
    ...
    override fun keyTyped(typedChar: Char, keyCode: Int) {
        when (keyCode) {
            Keyboard.KEY_DELETE -> if (Keyboard.KEY_DELETE == keyCode && selectedElement != null) {
                FDPClientChina.hud.removeElement(selectedElement!!)
            }
            ...
            else -> FDPClientChina.hud.handleKey(typedChar, keyCode)
        }
