...
class GuiBackground(val prevGui: GuiScreen) : GuiScreen() {
    ...
    private fun updateButtons() {
        ...
        val hasCustomBackground = FDPClientChina.background != null
    ...
    override fun actionPerformed(button: GuiButton) {
        when (button.id) {
            ...
            3 -> {
                ...
                try {
                    Files.copy(file.toPath(), FileOutputStream(FDPClientChina.fileManager.backgroundFile))

                    val image = ImageIO.read(FileInputStream(FDPClientChina.fileManager.backgroundFile))
                    FDPClientChina.background = ResourceLocation(FDPClientChina.CLIENT_NAME.lowercase() + "/background.png")
                    mc.textureManager.loadTexture(FDPClientChina.background, DynamicTexture(image))
                } catch (e: Exception) {
                    e.printStackTrace()
                    MiscUtils.showErrorPopup("Error", "Exception class: " + e.javaClass.name + "\nMessage: " + e.message)
                    FDPClientChina.fileManager.backgroundFile.delete()
                }
            }
            4 -> {
                FDPClientChina.background = null
                FDPClientChina.fileManager.backgroundFile.delete()
            }
            5 -> {
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        mc.fontRendererObj.drawCenteredString("%ui.background.gcurrent%: " + if(FDPClientChina.background == null) { GradientBackground.nowGradient.name } else { "Customized" },
