    ...
    override fun initGui() {
        server = MicrosoftAccount.Companion.buildFromOpenBrowser(object : MicrosoftAccount.OAuthHandler {
            ...
            override fun authResult(account: MicrosoftAccount) {
                if (FDPClientChina.fileManager.accountsConfig.altManagerMinecraftAccounts.any { it.name == account.name }) {
                    stage = "§c%ui.alt.alreadyAdded%"
                    return
                }
                FDPClientChina.fileManager.accountsConfig.altManagerMinecraftAccounts.add(account)
