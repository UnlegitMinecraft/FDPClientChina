...
class GuiAdd(private val prevGui: GuiAltManager) : GuiScreen() {
    ...
    override fun actionPerformed(button: GuiButton) {
        ...
        when (button.id) {
            ...
            1 -> {
                if (FDPClientChina.fileManager.accountsConfig.altManagerMinecraftAccounts.any { it.name == username.text }) {
                    status = "§c%ui.alt.alreadyAdded%"
                    return
                }
                FDPClientChina.fileManager.accountsConfig.altManagerMinecraftAccounts.add(AccountSerializer.accountInstance(username.text, password.text))
